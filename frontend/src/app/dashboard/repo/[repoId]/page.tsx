...
import { scansApi as scansApiType } from "@/lib/api";
...
export default function RepoDashboardPage() {
  ...
  const [activeJobTab, setActiveJobTab] = useState<"dashboard" | "chat">("dashboard");
  ...
  function onScanComplete(scanId: string) {
    setScanning(false);
    setCurrentJobId(null);
    scansApi.getScanResult(scanId).then(setScanResult).catch(() => {});
  }
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={() => setActiveJobTab(activeJobTab === "dashboard" ? "chat" : "dashboard")}
          ...
          >
            {activeJobTab === "dashboard" ? "Chat Agent" : "Dashboard"}
          </button>
          <button
            onClick={startScan}
            disabled={scanning}
            ...
            <>
              ...
              <div className="flex flex-col lg:flex-row gap-6 items-center">
                <div className="flex flex-wrap gap-8 justify-center lg:justify-start">
                  <RingScore label="Security" score={scanResult.scores.security} color="#ef4444" />
                  <RingScore label="Bloat" score={scanResult.scores.bloat} color="#f97316" />
                  <RingScore label="Freshness" score={scanResult.scores.freshness} color="#00d4ff" />
                  <RingScore label="Best Practices" score={scanResult.scores.bestPractices} color="#8b5cf6" />
                </div>
              ...
              <div className="bg-bg-card border border-surface-border rounded-xl overflow-hidden">
                <div className="px-6 py-4 border-b border-surface-border">
                  ...
                  <p className="text-text-secondary text-xs mt-0.5">
                    {scanResult.findings.length} findings &bull; {scanResult.estimatedFixTime}h estimated fix time &bull; {(scanResult.totalSizeReduction / 1024 / 1024).toFixed(0)}MB potential reduction
                  </p>
                </div>
